...
function div(atributos, info, acc){
    //console.log(acc);
    let cod = ""
    if(quitarEspacioInicial(acc) == "si"){
        cod = `
        <div ${atributos}>
            ${info}
        </div>
...
function imagen(link, style, clas, events, id, animation){
    //console.log(style);
    let cod = `
    <img id="${quitarEspacioInicial(descomponerArreglo(id))}" ${descomponerArreglo(animation)} src="${descomponerArreglo(link)}" style="${descomponerArreglo(style)}" class="${descomponerArreglo(clas)}" ${descomponerArreglo(events)} alt="">
    `
    return cod;
}
